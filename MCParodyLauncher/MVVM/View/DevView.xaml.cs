    ...
    public partial class DevView : UserControl
    {
        ...
        private void CheckInstall()
        {
            try
            {
                ...
                if (obGameInstalled != null)
                {
                    ...
                    else { Status = GameStatus.noInstall; }
                }
                else { Status = GameStatus.noInstall; }
                keyGame.Close();
                return;
        ...
        private async Task CheckForUpdates()
        {
            ...
            try
            {
                ...
                if (onlineVer.IsDifferentThan(localVer)) { updateAvailable = true; Status = GameStatus.update; }
                else { updateAvailable = false; Status = GameStatus.ready; }
                return;
            }
            catch (Exception ex) { ErrorMSG(ex); return; }
        }
        ...
        private void UpdateNotif()
        {
            ...
            if (updateResult == MessageBoxResult.Yes) { Status = GameStatus.updating; DownloadGame(); }
            else { return; }
        }
        ...
        {
            try
            {   if (Status == GameStatus.noInstall)
                {
                ...
                if (Status == GameStatus.downloading || Status == GameStatus.updating) { return; }
        ...
        private async Task ExtractZipAsync(string zipfile, string output)
        {
            try
            {
                ...
                {
                    string dlStatus;
                    if (Status == GameStatus.updating) { dlStatus = "updating"; }
        ...
        private void ErrorMSG (Exception exception) { Status = GameStatus.error; Dispatcher.BeginInvoke(new Action(() => System.Windows.MessageBox.Show($"{exception}", "Error", MessageBoxButton.OK, MessageBoxImage.Error)), System.Windows.Threading.DispatcherPriority.Normal);  return; }
        ...
        private void CheckForUpdatesBTN_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                CheckForUpdates();
                MessageBox.Show("No updates are available.", $"{GameName}", MessageBoxButton.OK, MessageBoxImage.Information);
            }
            catch (Exception ex) { ErrorMSG(ex); return; }
        }
